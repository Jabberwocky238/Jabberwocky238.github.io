...
在 Rust 中，`#[non_exhaustive]` 是一个属性，用于指示一个类型（通常是枚举或结构体）的定义是不完整的，并且未来可能会添加更多的变体或字段，而不会破坏二进制兼容性。这个属性表明类型是“非穷尽的”（non-exhaustive），意味着你不能安全地使用 `match` 语句或模式匹配来覆盖所有的情况。
...
在上面的例子中，即使 `handle_message` 函数试图覆盖所有现有的 `Message` 枚举变体，它仍然无法编译，因为 `Message` 被标记为 `#[non_exhaustive]`，编译器不允许进行穷尽性检查。

#### 注意事项

- **不适用于所有场景**：
  - `#[non_exhaustive]` 主要用于库的公共API，以确保未来可以安全地添加新的变体或字段。

- **不影响内部使用**：
  - 如果你在自己的库内部使用 `#[non_exhaustive]` 类型，你可以在库的内部代码中进行穷尽性检查，因为 `#[non_exhaustive]` 只影响外部代码。

- **与 Rust 的版本有关**：
  - `#[non_exhaustive]` 属性是在 Rust 1.53 版本中稳定下来的，所以需要至少这个版本的 Rust 来使用它。

#### 总结

`#[non_exhaustive]` 是 Rust 提供的一个有用的属性，它有助于在设计库时保持灵活性，同时避免未来不兼容的变更。
...
1. **内存布局的控制**：
   - 使用 `#[repr(C)]` 可以精确地控制结构体或枚举在内存中的排列方式，使之与 C 语言的布局相同。

2. **提高与 C 语言的兼容性**：
   - 当你需要与 C 语言库进行交互时，使用 `#[repr(C)]` 可以确保 Rust 代码中的类型与 C 代码中的类型在内存中布局一致，从而可以直接在 C 代码中使用这些类型。

3. **避免内存对齐**：
   - 默认情况下，Rust 编译器会对结构体成员进行对齐以提高访问效率，但这可能会增加结构体的大小。使用 `#[repr(C)]` 可以避免这种自动对齐，使得结构体的内存布局更加紧凑。
...
在这个例子中，`Point` 结构体和 `Color` 枚举被指定为具有 C 语言的内存布局。这意味着它们可以被直接传递给 C 语言代码，或者从 C 语言代码中读取，而无需进行任何转换。

### 注意事项

- **与 Rust 的内存安全理念相悖**：
  - 使用 `#[repr(C)]` 可能会使得代码更难写正确，因为它绕过了 Rust 默认的内存安全保证，如所有权和借用规则。

- **可能影响性能**：
  - 虽然 `#[repr(C)]` 可以使得结构体更紧凑，但也可能影响缓存的利用和访问速度，因为数据可能不会被自然对齐。

- **使用场景**：
  - 主要用于需要与 C 语言代码交互的情况，或者需要精确控制内存布局的低级编程。

#### 总结

`#[repr(C)]` 是一个强大的属性，它允许 Rust 代码与 C 语言代码在内存布局上保持一致，从而方便进行外部函数接口调用。然而，使用它需要谨慎，因为它可能会使得代码更难维护且影响性能。
